...
pub const CallState = struct {
    ...
    pub fn deinit(self: *CallState) void {
        ...
        // Free capsule storage
        var capsule_iter = self.capsule_storage.iterator();
        while (capsule_iter.next()) |entry| {
            self.capsule_storage.allocator.free(entry.key_ptr.*);
            self.capsule_storage.allocator.free(entry.value_ptr.*);
        }
    ...
    pub fn mergeChild(self: *CallState, child: *CallState) !void {
        ...
        if (!child.is_static_call) {
            ...
            while (capsule_iter.next()) |entry| {
                const key_copy = try self.capsule_storage.allocator.dupe(u8, entry.key_ptr.*);
                const value_copy = try self.capsule_storage.allocator.dupe(F, entry.value_ptr.*);
                try self.capsule_storage.put(key_copy, value_copy);
            }
        }
    ...
    /// Look up capsule storage, checking parent chain if not found locally
    pub fn getCapsule(self: *CallState, key: []const u8) ?[]F {
        ...
        // Check parent chain
        if (self.parent) |parent| {
            return parent.getCapsule(key);
        }
        
        return null;
    }
    
    /// Store capsule data in the current context
    pub fn storeCapsule(self: *CallState, key: []const u8, capsule: []const F) !void {
        ...
        // Store new data
        const key_copy = try self.capsule_storage.allocator.dupe(u8, key);
        const capsule_copy = try self.capsule_storage.allocator.alloc(F, capsule.len);
        @memcpy(capsule_copy, capsule);
        
        try self.capsule_storage.put(key_copy, capsule_copy);
    }
