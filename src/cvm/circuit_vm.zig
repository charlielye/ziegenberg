...
pub fn CircuitVm(Dispatcher: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn executeVm(self: *Self, function_index: usize, options: ExecuteOptions) !void {
            for (self.program.functions[function_index].opcodes, 0..) |opcode, i| {
                ...
                switch (opcode) {
                    ...
                    .BrilligCall => |op| {
                        ...
                        defer arena.deinit();
                        var brillig_vm = try bvm.BrilligVm(Dispatcher).init(arena.allocator(), calldata.items, self.fc_handler);
        ...
        inline fn resolveFunctionInput(self: *Self, comptime T: type, fi: io.FunctionInput) T {
