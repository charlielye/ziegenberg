...
pub fn computeOvskApp(ovsk: GrumpkinScalar, app: AztecAddress) !Fq {
    const ovsk_app_fr = try computeAppSecretKey(ovsk, app, .ov);
...
fn sha512ToGrumpkinScalar(inputs: []const u8) GrumpkinScalar {
    ...
    // Take first 32 bytes and convert to GrumpkinScalar
    var scalar_bytes: [32]u8 = undefined;
    @memcpy(&scalar_bytes, hash_output[0..32]);
    return GrumpkinScalar.from_buf(scalar_bytes);
}

pub fn deriveMasterNullifierSecretKey(secret_key: Fr) GrumpkinScalar {
    var input_buf: [36]u8 = undefined;
    secret_key.to_buf(input_buf[0..32]);
    std.mem.writeInt(u32, input_buf[32..36], @intFromEnum(constants.GeneratorIndex.nsk_m), .big);
...
pub fn deriveMasterIncomingViewingSecretKey(secret_key: Fr) GrumpkinScalar {
    var input_buf: [36]u8 = undefined;
    secret_key.to_buf(input_buf[0..32]);
    std.mem.writeInt(u32, input_buf[32..36], @intFromEnum(constants.GeneratorIndex.ivsk_m), .big);
    return sha512ToGrumpkinScalar(&input_buf);
}
...
pub fn deriveMasterOutgoingViewingSecretKey(secret_key: Fr) GrumpkinScalar {
    var input_buf: [36]u8 = undefined;
    secret_key.to_buf(input_buf[0..32]);
    std.mem.writeInt(u32, input_buf[32..36], @intFromEnum(constants.GeneratorIndex.ovsk_m), .big);
    return sha512ToGrumpkinScalar(&input_buf);
}
...
pub fn computePreaddress(public_keys_hash: Fr, partial_address: Fr) !Fr {
    const inputs = [_]Fr{ public_keys_hash, partial_address, Fr.from_int(@intFromEnum(constants.GeneratorIndex.contract_address_v1)) };
    return poseidon2.hash(&inputs);
}
...
pub fn computeAddress(public_keys: PublicKeys, partial_address: Fr) !AztecAddress {
    ...
    const preaddress = try computePreaddress(try public_keys.hash(), partial_address);
    const preaddress_point = try derivePublicKeyFromSecretKey(Fq.from_int(preaddress.to_int()));
...
pub fn computeAddressSecret(preaddress: Fr, ivsk: Fq) !Fq {
    ...
    const address_point_candidate = try derivePublicKeyFromSecretKey(address_secret_candidate);
...
pub fn derivePublicKeyFromSecretKey(secret_key: Fq) !G1.Element {
...
pub fn deriveKeys(secret_key: Fr) !DerivedKeys {
    ...
    var tsk_input_buf: [36]u8 = undefined;
    secret_key.to_buf(tsk_input_buf[0..32]);
    std.mem.writeInt(u32, tsk_input_buf[32..36], @intFromEnum(constants.GeneratorIndex.tsk_m), .big);
    const master_tagging_secret_key = sha512ToGrumpkinScalar(&tsk_input_buf);

    // Then we derive master public keys
    const master_nullifier_public_key = try derivePublicKeyFromSecretKey(master_nullifier_secret_key);
    const master_incoming_viewing_public_key = try derivePublicKeyFromSecretKey(master_incoming_viewing_secret_key);
    const master_outgoing_viewing_public_key = try derivePublicKeyFromSecretKey(master_outgoing_viewing_secret_key);
    const master_tagging_public_key = try derivePublicKeyFromSecretKey(master_tagging_secret_key);
...
fn computeTaggingSecretPoint(known_address: CompleteAddress, ivsk: Fq, external_address: AztecAddress) !G1.Element {
    const known_preaddress = try computePreaddress(try known_address.public_keys.hash(), known_address.partial_address);
    // TODO: #8970 - Computation of address point from x coordinate might fail
    const external_address_point = try external_address.toAddressPoint();
    // Given A (known complete address) -> B (external address) and h == preaddress
    // Compute shared secret as S = (h_A + ivsk_A) * Addr_Point_B
    const address_secret = try computeAddressSecret(known_preaddress, ivsk);
...
pub fn computeAppTaggingSecret(
    ...
    const tagging_secret_point = try computeTaggingSecretPoint(known_address, ivsk, external_address);
